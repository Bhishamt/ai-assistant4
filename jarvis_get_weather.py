...
def detect_city_by_ip() -> str:
    try:
        ...
        if city:
            ...
            logger.warning("City detect करने में विफल, default ' himachal pradesh Mandi, Thunag (175045)' इस्तेमाल किया जा रहा है।")
            return "Mandi, Thunag"
    except Exception as e:
        logger.error(f"IP से city detect करने में error आया: {e}")
        return "himachal pradesh, mandi, thunag (175045)"  # Default city if IP detection fails

def get_city_from_file() -> str:
    try:
        with open('addrees.json', 'r') as f:
            data = json.load(f)
            return data.get('city', '')
    except FileNotFoundError:
        logger.error("addrees.json not found.")
        return ""
    except json.JSONDecodeError:
        logger.error("Error decoding addrees.json.")
        return ""
